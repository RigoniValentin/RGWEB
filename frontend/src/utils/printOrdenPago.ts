/**
 * Orden de Pago — Browser Print API
 *
 * Generates an A4 HTML receipt for an Orden de Pago (supplier payment order)
 * and prints it via a hidden iframe using window.print().
 *
 * Follows the same pattern as printReciboCobranza.ts
 */

import type { OrdenPagoReciboData } from '../services/ordenesPago.api';
...
function buildOrdenPagoHTML(data: OrdenPagoReciboData, logoDataUrl?: string | null): string {
  ...
  // Parse concepto (strip "OP #ID - " prefix)
...
<head>
  <meta charset="utf-8" />
  <title>Orden de Pago ${String(data.PAGO_ID).padStart(6, '0')} - ${data.PROVEEDOR_NOMBRE} - ${formatDateShort(data.FECHA)}</title>
  <style>
    @page {
      size: A4 portrait;
      margin: 20mm;
...
<body>
  ...
  <div class="recibo-header">
    ...
    <div class="recibo-numero-box">
      <div class="recibo-label">ORDEN DE PAGO Nº</div>
  ...
  <div class="client-box">
    <div class="client-grid">
      <div class="field">
        ...
        <span class="value">${escapeHTML(data.PROVEEDOR_NOMBRE)}</span>
      </div>
      <div class="field">
        <span class="label">Código:</span>
        <span class="value">${escapeHTML(data.PROVEEDOR_CODIGO || '-')}</span>
      </div>
      ${data.PROVEEDOR_DOMICILIO ? `
      <div class="field">
        <span class="label">Dirección:</span>
        <span class="value">${escapeHTML(data.PROVEEDOR_DOMICILIO)}</span>
      </div>` : ''}
      ${data.PROVEEDOR_LOCALIDAD ? `
      <div class="field">
        <span class="label">Localidad:</span>
        <span class="value">${escapeHTML(data.PROVEEDOR_LOCALIDAD)}</span>
      </div>` : ''}
      ${data.PROVEEDOR_DOCUMENTO ? `
      <div class="field">
        <span class="label">CUIT/DNI:</span>
        <span class="value">${escapeHTML(data.PROVEEDOR_DOCUMENTO)}</span>
      </div>` : ''}
    </div>
  </div>
  ...
  <div class="recibo-footer">
    <div class="firma-box">
      ...
      <div class="firma-text">Conforme</div>
    </div>
  </div>
...
/**
 * Fetches orden de pago data and opens a print dialog.
 * Uses a hidden iframe to avoid disrupting the current page.
 */
export async function printOrdenPago(data: OrdenPagoReciboData): Promise<void> {
  ...
  const html = buildOrdenPagoHTML(data, logoDataUrl);
