...
import {
  libroIvaComprasApi,
  type LibroIvaComprasComprobante,
  type LibroIvaComprasAlicuota,
  type LibroIvaComprasFilter,
...
export function LibroIvaComprasPage() {
  ...
  const filter: LibroIvaComprasFilter = useMemo(() => ({
    fechaDesde,
    fechaHasta,
    puntoVentaId,
  ...
  const { data: comprobantes, isLoading, refetch } = useQuery({
    queryKey: ['libro-iva-compras-comprobantes', filter],
    queryFn: () => libroIvaComprasApi.getComprobantes(filter),
    enabled: !!fechaDesde && !!fechaHasta,
  });

  const { data: totales } = useQuery({
    queryKey: ['libro-iva-compras-totales', filter],
    queryFn: () => libroIvaComprasApi.getTotales(filter),
    enabled: !!fechaDesde && !!fechaHasta,
  });

  const { data: alicuotas } = useQuery({
    queryKey: ['libro-iva-compras-alicuotas', filter],
    queryFn: () => libroIvaComprasApi.getAlicuotas(filter),
    enabled: !!fechaDesde && !!fechaHasta,
  });

  const { data: puntosVenta } = useQuery({
    queryKey: ['libro-iva-compras-puntos-venta'],
    queryFn: () => libroIvaComprasApi.getPuntosDeVenta(),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const handleExportExcel = () => {
    ...
    const headers = [
      'Fecha', 'Tipo Comp.', 'PV', 'Número', 'CAE', 'Proveedor', 'CUIT',
      'Neto No Gravado', 'Neto Gravado', 'IVA', 'Imp. Interno',
      'Perc. IVA', 'Perc. IIBB', 'Total',
    ];
    const rows = comprobantes.map(c => [
      ...
      c.PERCEPCION_IVA.toFixed(2),
      c.PERCEPCION_IIBB.toFixed(2),
      c.TOTAL.toFixed(2),
    ]);
  ...
  const handleExportCiti = async () => {
    ...
    try {
      const data = await libroIvaComprasApi.exportCiti(filter);
      const periodo = dayjs(fechaDesde).format('YYYYMM');
      downloadFile(data.comprobantes, `COMPRAS_CBTE_${periodo}.txt`, 'text/plain');
      setTimeout(() => {
  ...
  const columns: TableColumnType<LibroIvaComprasComprobante>[] = [
    {
      title: 'Fecha',
      dataIndex: 'FECHA',
      width: 95,
    ...
    },
    {
      ...
      align: 'right',
      render: (v: number) => (
        <Text style={{ color: '#722ed1', fontSize: 12.5 }}>{fmtMoney(v)}</Text>
      ),
    ...
    },
    {
      ...
      render: (_: unknown, record) => {
        const total = (record.PERCEPCION_IVA || 0) + (record.PERCEPCION_IIBB || 0);
        return total === 0 ? (
          <Text type="secondary" style={{ fontSize: 12.5 }}>—</Text>
        ) : (
          <Tooltip title={`IVA: ${fmtMoney(record.PERCEPCION_IVA || 0)} | IIBB: ${fmtMoney(record.PERCEPCION_IIBB || 0)}`}>
            <Text style={{ fontSize: 12.5, color: '#fa8c16' }}>{fmtMoney(total)}</Text>
          </Tooltip>
        );
      },
    },
  ...
  // ── Columnas alícuotas ───────────────────────────
  const alicuotaColumns: TableColumnType<LibroIvaComprasAlicuota>[] = [
    {
      title: 'Alícuota',
    ...
    },
    {
      ...
      align: 'center',
      render: (v: number) => (
        <Badge count={v} color="#722ed1" overflowCount={9999} />
      ),
    },
    {
    ...
    },
    {
      ...
      dataIndex: 'CREDITO_FISCAL',
      width: 130,
      align: 'right',
      render: (v: number) => (
        <Text strong style={{ color: '#722ed1' }}>{fmtMoney(v)}</Text>
      ),
    },
  ];
  ...
  return (
    <div className="page-enter">
      ...
      <div className="page-header">
        <div>
          <Title level={3}>
            <ShoppingCartOutlined style={{ marginRight: 10, color: 'var(--rg-gold)' }} />
            Libro IVA Compras
          </Title>
        ...
        <Space>
          ...
          <Tooltip title="Generar archivos TXT para importar en AFIP CITI Compras">
            <Button
      ...
      <Row gutter={[12, 12]} className="stagger" style={{ marginBottom: 16 }}>
        ...
        <Col xs={12} sm={8} lg={4}>
          <Card size="small" className="rg-card animate-fade-up">
            ...
            <div style={{ marginTop: 6, display: 'flex', gap: 6, alignItems: 'center' }}>
              <Text type="secondary" style={{ fontSize: 11 }}>
                IVA {fmtMoney(totales?.TOTAL_PERCEPCION_IVA ?? 0)}
              </Text>
              <Divider type="vertical" style={{ margin: 0 }} />
              <Text type="secondary" style={{ fontSize: 11 }}>
                IIBB {fmtMoney(totales?.TOTAL_PERCEPCION_IIBB ?? 0)}
              </Text>
            </div>
          </Card>
        </Col>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            />
            <Text type="secondary" style={{ fontSize: 12 }}>Incluir no pagadas</Text>
          </div>
          <div style={{ flex: 1 }} />
          <Text type="secondary" style={{ fontSize: 12 }}>
      ...
      >
        <Table<LibroIvaComprasComprobante>
          className="rg-table"
          rowKey="COMPRA_ID"
          columns={columns}
        ...
        <Col xs={24} lg={10}>
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '4px 0' }}>
              ...
              <SummaryRow label="IVA Crédito Fiscal" value={totales?.TOTAL_IVA ?? 0} color="#722ed1" />
              <SummaryRow label="Imp. Internos"    value={totales?.TOTAL_IMPUESTO_INTERNO ?? 0} />
              <SummaryRow label="Percep. IVA"      value={totales?.TOTAL_PERCEPCION_IVA   ?? 0} color="#fa8c16" />
              <SummaryRow label="Percep. IIBB"     value={totales?.TOTAL_PERCEPCION_IIBB  ?? 0} color="#fa8c16" />
