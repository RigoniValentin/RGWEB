-- =============================================================================
-- MIGRACIÓN: Poblar COMPRAS_METODOS_PAGO para compras existentes
-- =============================================================================
...
-- Solo procesa compras que NO tienen registros en la tabla nueva.
-- =============================================================================

BEGIN TRANSACTION;

-- Asegurar que la tabla destino existe
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'COMPRAS_METODOS_PAGO')
BEGIN
...
-- Insertar EFECTIVO para compras que tienen monto en efectivo y no tienen ya ese método
INSERT INTO COMPRAS_METODOS_PAGO (COMPRA_ID, METODO_PAGO_ID, MONTO)
SELECT c.COMPRA_ID, @efectivoId, c.MONTO_EFECTIVO
FROM COMPRAS c
WHERE c.MONTO_EFECTIVO <> 0
  AND NOT EXISTS (
      SELECT 1 FROM COMPRAS_METODOS_PAGO cmp
      WHERE cmp.COMPRA_ID = c.COMPRA_ID AND cmp.METODO_PAGO_ID = @efectivoId
  );

DECLARE @rowsEfectivo INT = @@ROWCOUNT;
PRINT 'Registros insertados (Efectivo): ' + CAST(@rowsEfectivo AS VARCHAR);

-- Insertar TRANSFERENCIA para compras que tienen monto digital y no tienen ya ese método
INSERT INTO COMPRAS_METODOS_PAGO (COMPRA_ID, METODO_PAGO_ID, MONTO)
SELECT c.COMPRA_ID, @transferenciaId, c.MONTO_DIGITAL
FROM COMPRAS c
WHERE c.MONTO_DIGITAL <> 0
  AND NOT EXISTS (
      SELECT 1 FROM COMPRAS_METODOS_PAGO cmp
      WHERE cmp.COMPRA_ID = c.COMPRA_ID AND cmp.METODO_PAGO_ID = @transferenciaId
  );
