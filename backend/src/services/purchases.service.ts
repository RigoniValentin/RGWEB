...
async function incrementarStock(
  ...
  if (esConjunto) {
    ...
    for (const child of children.recordset) {
      ...
      const childExistsI = await tx.request()
      ...
      if (childExistsI.recordset.length > 0) {
        await tx.request()
          .input('prodId', sql.Int, child.PRODUCTO_ID_HIJO)
          .input('depId', sql.Int, child.DEPOSITO_ID)
    ...
    if (descuentaStock) {
      if (depositoId) {
        ...
        const parentExistsI = await tx.request()
          .input('prodId', sql.Int, productoId)
          .input('depId', sql.Int, depositoId)
          .query('SELECT 1 AS E FROM STOCK_DEPOSITOS WHERE PRODUCTO_ID = @prodId AND DEPOSITO_ID = @depId');
        if (parentExistsI.recordset.length > 0) {
          await tx.request()
            .input('prodId', sql.Int, productoId)
            .input('depId', sql.Int, depositoId)
    ...
    if (depositoId) {
      const prevStock = await getCurrentStock(tx, productoId, depositoId);
      const depExistsI = await tx.request()
        .input('prodId', sql.Int, productoId)
        .input('depId', sql.Int, depositoId)
        .query('SELECT 1 AS E FROM STOCK_DEPOSITOS WHERE PRODUCTO_ID = @prodId AND DEPOSITO_ID = @depId');
      if (depExistsI.recordset.length > 0) {
        await tx.request()
          .input('prodId', sql.Int, productoId)
          .input('depId', sql.Int, depositoId)
          .input('cant', sql.Decimal(18, 4), cantidad)
  ...
  if (esConjunto) {
    ...
    for (const child of children.recordset) {
      ...
      const childExistsD = await tx.request()
        ...
        .input('depId', sql.Int, child.DEPOSITO_ID)
        .query('SELECT 1 AS E FROM STOCK_DEPOSITOS WHERE PRODUCTO_ID = @prodId AND DEPOSITO_ID = @depId');
      if (childExistsD.recordset.length > 0) {
        await tx.request()
          .input('prodId', sql.Int, child.PRODUCTO_ID_HIJO)
          .input('depId', sql.Int, child.DEPOSITO_ID)
          .input('cant', sql.Decimal(18, 4), childQty)
    ...
    if (descuentaStock) {
      if (depositoId) {
        ...
        const parentExistsD = await tx.request()
          ...
          .input('depId', sql.Int, depositoId)
          .query('SELECT 1 AS E FROM STOCK_DEPOSITOS WHERE PRODUCTO_ID = @prodId AND DEPOSITO_ID = @depId');
        if (parentExistsD.recordset.length > 0) {
          await tx.request()
            .input('prodId', sql.Int, productoId)
            .input('depId', sql.Int, depositoId)
            .input('cant', sql.Decimal(18, 4), cantidad)
    ...
    if (depositoId) {
      ...
      const depExistsD = await tx.request()
        ...
        .input('depId', sql.Int, depositoId)
        .query('SELECT 1 AS E FROM STOCK_DEPOSITOS WHERE PRODUCTO_ID = @prodId AND DEPOSITO_ID = @depId');
      if (depExistsD.recordset.length > 0) {
        await tx.request()
          .input('prodId', sql.Int, productoId)
          .input('depId', sql.Int, depositoId)
          .input('cant', sql.Decimal(18, 4), cantidad)
...
async function actualizarPreciosVenta(
  ...
  if (margenIndividual) {
    ...
    const margenes = await tx.request()
      .input('pid', sql.Int, productoId)
      .query(`
        SELECT LISTA_ID, MARGEN_INDIVIDUAL
        FROM PRODUCTO_LISTA_PRECIOS
        WHERE PRODUCTO_ID = @pid AND MARGEN_INDIVIDUAL IS NOT NULL
      `);
    ...
    for (const row of margenes.recordset as { LISTA_ID: number; MARGEN_INDIVIDUAL: number }[]) {
      ...
      if (precio > 0) {
        await tx.request()
          .input('pid', sql.Int, productoId)
          .input('listaId', sql.Int, row.LISTA_ID)
          .input('precio', sql.Decimal(18, 4), precio)
          .input('margen', sql.Decimal(9, 4), margen)
          .query(`
            ...
            WHEN MATCHED THEN
              UPDATE SET PRECIO = src.PRECIO, MARGEN_INDIVIDUAL = @margen, FECHA_ACTUALIZACION = GETDATE()
            WHEN NOT MATCHED THEN
              INSERT (PRODUCTO_ID, LISTA_ID, PRECIO, MARGEN_INDIVIDUAL)
              VALUES (src.PRODUCTO_ID, src.LISTA_ID, src.PRECIO, @margen);
          `);
      }
    }
  } else {
...
export const purchasesService = {
  ...
  async create(input: CompraInput, usuarioId: number) {
    ...
    try {
      ...
      if (!input.ES_CTA_CORRIENTE) {
        ...
        // ── Cheques: marcar EN_CARTERA → EGRESADO si se proveyó cheques_ids ──
        let montoChequesEgresados = 0;
        if (input.cheques_ids && input.cheques_ids.length > 0) {
          const provNombreRow = await tx.request()
            .input('pid', sql.Int, input.PROVEEDOR_ID)
            .query(`SELECT NOMBRE FROM PROVEEDORES WHERE PROVEEDOR_ID = @pid`);
          const nombreProv = provNombreRow.recordset[0]?.NOMBRE || '';
          const result = await marcarChequesEgresados(
            tx, input.cheques_ids, 'COMPRA', compraId,
            `Pago compra #${compraId} - ${nombreProv}`,
            usuarioId, null,
          );
          montoChequesEgresados = result.total;
          // Validar consistencia con el aporte declarado en metodos_pago (si lo hay)
          if (montoChequesAporte > 0 && Math.abs(montoChequesEgresados - montoChequesAporte) > 0.01) {
            throw Object.assign(
              new Error(`La suma de los cheques egresados ($${montoChequesEgresados}) no coincide con el aporte declarado en métodos de pago ($${montoChequesAporte})`),
              { name: 'ValidationError' }
            );
          }
        }

        if (efectivoNeto > 0 || montoDigital > 0 || montoChequesEgresados > 0) {
    ...
    try {
      ...
      try {
        const ncCheckUpd = await tx.request()
          ...
          .query(`SELECT COUNT(*) AS CNT FROM NC_COMPRAS WHERE COMPRA_ID = @compraId AND ANULADA = 0`);
        if (ncCheckUpd.recordset[0].CNT > 0) {
          throw Object.assign(
            new Error('No se puede modificar la compra porque tiene notas de crédito activas. Anule las NC primero.'),
            { name: 'ValidationError' }
          );
        }
      } catch (ncErrUpd2: any) {
        if (ncErrUpd2.name === 'ValidationError') throw ncErrUpd2;
        // If NC_COMPRAS table doesn't exist yet, ignore
      }
      ...
      // First revert any egresado cheques (return them to EN_CARTERA)
      try {
        await revertirEgresoCheques(tx, 'COMPRA', id, usuarioId, null);
      ...
      if (!input.ES_CTA_CORRIENTE) {
        ...
        // Re-mark cheques as EGRESADO if provided
        let montoChequesEgresados = 0;
        if (input.cheques_ids && input.cheques_ids.length > 0) {
          const provNombreChq = await tx.request()
            .input('pid', sql.Int, input.PROVEEDOR_ID)
            .query(`SELECT NOMBRE FROM PROVEEDORES WHERE PROVEEDOR_ID = @pid`);
          const nombreProvChq = provNombreChq.recordset[0]?.NOMBRE || '';
          const chqResult = await marcarChequesEgresados(
            tx, input.cheques_ids, 'COMPRA', id,
            `Pago compra #${id} - ${nombreProvChq}`,
            usuarioId, null,
          );
  ...
  async delete(id: number, usuarioId: number) {
    ...
    try {
      ...
      try {
        const ncCheckDel = await tx.request()
          .input('compraId', sql.Int, id)
          .query(`SELECT COUNT(*) AS CNT FROM NC_COMPRAS WHERE COMPRA_ID = @compraId AND ANULADA = 0`);
        if (ncCheckDel.recordset[0].CNT > 0) {
          throw Object.assign(
            new Error('No se puede eliminar la compra porque tiene notas de crédito activas. Anule las NC primero.'),
            { name: 'ValidationError' }
          );
        }
      } catch (ncErrDel: any) {
        if (ncErrDel.name === 'ValidationError') throw ncErrDel;
        // If NC_COMPRAS table doesn't exist yet, ignore
      }
      ...
      // ── 4b. Reversa de cheques egresados (vuelven a EN_CARTERA) ──
      try {
        await revertirEgresoCheques(tx, 'COMPRA', id, usuarioId, null);
      } catch { /* tabla puede no existir aún */ }
  ...
  async getPriceCheckData(compraId: number) {
    ...
    // 3. Get product data + precios pivotados como JSON desde PRODUCTO_LISTA_PRECIOS.
    //    Estructura nueva: precios = [{ LISTA_ID, PRECIO, MARGEN_INDIVIDUAL }, ...]
    ...
    const productsResult = await pool.request()
      ...
      .query(`
        SELECT
          ...
          p.LISTA_DEFECTO,
          CASE WHEN EXISTS (
            SELECT 1 FROM PRODUCTO_LISTA_PRECIOS plp
            WHERE plp.PRODUCTO_ID = p.PRODUCTO_ID AND plp.MARGEN_INDIVIDUAL IS NOT NULL
          ) THEN 1 ELSE 0 END AS TIENE_MARGENES_INDIV,
          (
            SELECT plp.LISTA_ID AS LISTA_ID,
                   ISNULL(plp.PRECIO, 0) AS PRECIO,
                   plp.MARGEN_INDIVIDUAL AS MARGEN_INDIVIDUAL
    ...
    const products = (productsResult.recordset as any[]).map(row => {
      let precios: { LISTA_ID: number; PRECIO: number; MARGEN_INDIVIDUAL: number | null }[] = [];
      if (row.PRECIOS_JSON) {
  ...
  async savePriceCheck(updates: { PRODUCTO_ID: number; precios: { LISTA_ID: number; PRECIO: number }[] }[]) {
    ...
    try {
      // Cargar márgenes default de todas las listas una sola vez
      const listasInfo = await tx.request()
        .query(`SELECT LISTA_ID, ISNULL(MARGEN, 0) AS MARGEN FROM LISTA_PRECIOS`);
      const margenDefault: Record<number, number> = {};
      for (const l of listasInfo.recordset as { LISTA_ID: number; MARGEN: number }[]) {
        margenDefault[l.LISTA_ID] = l.MARGEN || 0;
      }
      ...
      for (const item of updates) {
        ...
        // Upsert en PRODUCTO_LISTA_PRECIOS para cada precio enviado.
        // MARGEN_INDIVIDUAL se setea automáticamente si el margen real difiere
        // del margen default de la lista (más allá de la tolerancia).
        for (const p of item.precios) {
          const precio = Number(p.PRECIO) || 0;
          if (precio > 0) {
            const margenReal = costo > 0
              ? Math.round(((precio / costo) - 1) * 100 * 10000) / 10000
              : 0;
            const margenDef = margenDefault[p.LISTA_ID] || 0;
            const margenIndividual = (costo > 0 && Math.abs(margenReal - margenDef) > 0.5)
              ? margenReal
              : null;

            await tx.request()
              .input('pid', sql.Int, item.PRODUCTO_ID)
              .input('listaId', sql.Int, p.LISTA_ID)
              .input('precio', sql.Decimal(18, 4), precio)
              .input('margenInd', sql.Decimal(9, 4), margenIndividual)
              .query(`
                MERGE PRODUCTO_LISTA_PRECIOS AS target
                USING (SELECT @pid AS PRODUCTO_ID, @listaId AS LISTA_ID, @precio AS PRECIO) AS src
                ON target.PRODUCTO_ID = src.PRODUCTO_ID AND target.LISTA_ID = src.LISTA_ID
                WHEN MATCHED THEN
                  UPDATE SET PRECIO = src.PRECIO,
                             MARGEN_INDIVIDUAL = @margenInd,
                             FECHA_ACTUALIZACION = GETDATE()
                WHEN NOT MATCHED THEN
                  INSERT (PRODUCTO_ID, LISTA_ID, PRECIO, MARGEN_INDIVIDUAL)
                  VALUES (src.PRODUCTO_ID, src.LISTA_ID, src.PRECIO, @margenInd);
              `);
          } else {
            await tx.request()
              .input('pid', sql.Int, item.PRODUCTO_ID)
              .input('listaId', sql.Int, p.LISTA_ID)
              .query('DELETE FROM PRODUCTO_LISTA_PRECIOS WHERE PRODUCTO_ID = @pid AND LISTA_ID = @listaId');
          }
        }

        // Flag derivado: PRODUCTOS.MARGEN_INDIVIDUAL = 1 si el producto tiene
        // al menos un margen override seteado en PRODUCTO_LISTA_PRECIOS.
        const hasOverride = await tx.request()
          .input('pid', sql.Int, item.PRODUCTO_ID)
          .query(`SELECT COUNT(*) AS n FROM PRODUCTO_LISTA_PRECIOS WHERE PRODUCTO_ID = @pid AND MARGEN_INDIVIDUAL IS NOT NULL`);
        await tx.request()
          .input('pid', sql.Int, item.PRODUCTO_ID)
          .input('flag', sql.Bit, hasOverride.recordset[0].n > 0 ? 1 : 0)
          .query(`UPDATE PRODUCTOS SET MARGEN_INDIVIDUAL = @flag WHERE PRODUCTO_ID = @pid`);
